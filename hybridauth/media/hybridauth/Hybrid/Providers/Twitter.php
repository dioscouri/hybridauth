...
class Hybrid_Providers_Twitter extends Hybrid_Provider_Model_OAuth1
{
	...
	*/
	function initialize()
	{
		...
		require_once Hybrid_Auth::$config["path_libraries"] . "Twitter/OAuth1ClientTwitter.php";

		// 3.1 - setup access_token if any stored
		if( $this->token( "access_token" ) ){
			$this->api = new OAuth1ClientTwitter( 
		...
		// 3.3 - instanciate OAuth client with client credentials
			...
			$this->api = new OAuth1ClientTwitter( $this->config["keys"]["key"], $this->config["keys"]["secret"] );
		}
